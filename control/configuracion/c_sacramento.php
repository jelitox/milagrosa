<?php 
session_start();
	include_once '../../modelo/configuracion/m_sacramento.php'; // incluyo clase del objeto sacramento
	$obj_modelo = new Sacramento;
	$tabla = 'tsacramento'; //tabla de la bd
	$campoest = 'estatus'; // campo estatus de la tabla
	$idcampo = 'idsacramento'; // id de tabla en la bd (identificador unico o llave primaria)
	...
	switch ($_POST['op']) {
		case 'Registrar':   //strtoupper(); es para convertir a mayuscula los string, todos los datos en una bd tipo string deben llegar a mayusculas para tener un standard de datos
			...
			$obj_modelo->mis = strtoupper($_POST['mis']);  // POST DEL INPUT DE LA VISTA
		...
		case 'Consultar':
			...
			$obj_row = $obj_modelo->row($obj_modelo->consulta_mod($tabla,'WHERE','idsacramento',$_POST['idreg']));
			//$fecha = $obj_modelo->cambiarfecha($obj_row['tipo'],'d-m-Y');
			$data = array(0 => $obj_row['nombre'], 1 => $obj_row['mision'], 2 => $obj_row['vision']);
