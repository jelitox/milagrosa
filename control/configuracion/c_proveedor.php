<?php 
	@session_start();
	include_once '../../modelo/configuracion/m_proveedor.php'; // incluyo clase del objeto sacramento
	$obj_modelo = new Proveedor;
	$tabla = 'tproveedor'; //tabla de la bd
	$campoest = 'estatus'; // campo estatus de la tabla
	$idcampo = 'idproveedor'; // id de tabla en la bd (identificador unico o llave primaria)
	...
	switch ($_POST['op']) {
		case 'Registrar':   //strtoupper(); es para convertir a mayuscula los string, todos los datos en una bd tipo string deben llegar a mayusculas para tener un standard de datos
			...
			$obj_modelo->rif = strtoupper($_POST['rif']);  // POST DEL INPUT DE LA VISTA
			$obj_modelo->raz = strtoupper($_POST['raz']); // POST DEL INPUT DE LA VISTA
			$obj_modelo->tel = strtoupper($_POST['tel']);   // POST DEL INPUT DE LA VISTA
			$obj_modelo->cor = strtoupper($_POST['cor']);  // POST DEL INPUT DE LA VISTA
			$obj_modelo->dir = strtoupper($_POST['dir']); // POST DEL INPUT DE LA VISTA
		...
		case 'Consultar':
			...
			$obj_row = $obj_modelo->row($obj_modelo->consulta_mod($tabla,'WHERE','idproveedor',$_POST['idreg']));
			//$fecha = $obj_modelo->cambiarfecha($obj_row['tipo'],'d-m-Y');
			$data = array(0 => $obj_row['nombre'], 1 => $obj_row['rif'], 2 => $obj_row['razonsocial'], 3 => $obj_row['telefono'], 4 => $obj_row['correo'], 5 => $obj_row['direccion']);
