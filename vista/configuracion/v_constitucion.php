<?php 
	@session_start();
	include_once ('../modelo/configuracion/m_constitucion.php'); // incluyo el modelo del maestro
	$obj_bd = new Constitucion; // instancia de la clase del objeto
	...
	$url_control =  "../control/configuracion/c_constitucion.php"; // url del controlador
	$url_reporte = "../control/controlreporte/c_reporteconstitucion.php"; // url del controlador del reporte
	// $sql = "";
...
<div class="col-sm-2 col-md-3 col-xs-3 content">
	<form action="" method="POST" id="formulario">
			<h3 >Constitución de grupos</h3><hr>
			...
			<div>
					...
					<table class="cell-border compact hover stripe " id="filtro">
						<thead>
							<tr>
								...
								<td class="negrita-suave">Grupo</td>
								<td class="negrita-suave" width="20%">Encargado</td>
						...
						<tbody>
							...
							<tr>
								...
								<td><?php echo $est; ?></td>
								<td id="boton_mda">
									<button type="button" onclick="modificar(<?php echo $row['idconstitucion'];?>,'<?php echo $url_control;?>')" id="mod" class="btn-peq-suceso mod" name="mod"  ><i class="fa fa-edit"></button></i>

									<!-- en caso del estatus sea 1 ó 0 se activan los botones -->
									<?php if($row['estatus']==1){?>
										<button type="button" class="btn-peq-alerta" id="des" value="Desactivar" onclick="des_act(this.value,'<?php echo $url_control; ?>','<?php echo $row['idconstitucion'] ?>','Estas seguro de Desactivar este registro?')"><i class="fa fa-close"></i></button>	
									<?php }else{ ?>
										<button type="button" class="btn-peq-alerta verde" value="Activar" id="act"  onclick="des_act(this.value,'<?php echo $url_control; ?>','<?php echo $row['idconstitucion'] ?>','Estas seguro de Activar este registro?')"><i class="fa fa-check"></i></button>	
									<?php }?>
								</td>
						  	</tr>
							<?php  $c++;}?>
					...
					<div  class="row ">
						...
						<div class="col-sm-2 col-md-5 col-xs-3">
							<div class="form-group ">
								<label for=""> Grupo: <span class="oblig">*</span> </label>
							<select class="form-control tipogrupo" name="combo" id="select3" onchange="vista_grupo(this.value)">
								<option value="" readonly>Seleccione</option>
								<?php 
									$sql="SELECT *FROM tgrupo where estatus=1"; //sql para el combo  
									$obj_bd->crear_combo($sql,'idgrupo','nombre','idgrupo'); 
								?>
								</select>
							</div>
              ...
              <input type="text" name="<?php echo $cam2; ?>" value="<?php  if(isset($obj_row[$cam2])) echo $obj_row[$cam2];?>" class="form-control requerido" id="texto1" required>
