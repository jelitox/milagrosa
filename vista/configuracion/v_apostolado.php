<?php 
	@session_start();
	include_once ('../modelo/configuracion/m_apostolado.php'); // incluyo el modelo del maestro
	$obj_bd = new Apostolado; // instancia de la clase del objeto
	$tablabd = "tgrupo"; // tabla en la base de datos
	
	// echo '<div class="alert alert-info" id="alerta_success"><span id="msj_alert">"'.$_SESSION['msj'].'"</span></div>';
	$url_control =  "../control/configuracion/c_apostolado.php"; // url del controlador
	$url_reporte = "../control/controlreporte/c_reporteapostolado.php"; // url del controlador del reporte
	// $sql = "";
					...
					<table class="cell-border compact hover stripe " id="filtro">
						...
						<tbody>
							...
							<tr>
								...
								<td><?php echo $est; ?></td>
								<td id="boton_mda">
									<button type="button" onclick="modificar(<?php echo $row['idgrupo'];?>,'<?php echo $url_control;?>')" id="mod" class="btn-peq-suceso mod" name="mod"  ><i class="fa fa-edit"></button></i>

									<!-- en caso del estatus sea 1 ó 0 se activan los botones -->
									<?php if($row['estatus']==1){?>
										<button type="button" class="btn-peq-alerta" id="des" value="Desactivar" onclick="des_act(this.value,'<?php echo $url_control; ?>','<?php echo $row['idgrupo'] ?>','Estas seguro de Desactivar este registro?')"><i class="fa fa-close"></i></button>	
									<?php }else{ ?>
										<button type="button" class="btn-peq-alerta verde" value="Activar" id="act"  onclick="des_act(this.value,'<?php echo $url_control; ?>','<?php echo $row['idgrupo'] ?>','Estas seguro de Activar este registro?')"><i class="fa fa-check"></i></button>	
									<?php }?>
								</td>
						  	</tr>
							<?php  $c++;}?>
